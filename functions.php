...
/*
//This function will update the all passwords to the secured form
$query = mysql_query('SELECT `id`, `password` FROM `user` WHERE `uname` != "" AND `password` != "" AND `activated` = "yes"');
$new_values = array();
if(!empty($query)){
	while ($row = mysql_fetch_array($query)) {				
		$new_password = sha1( $row['password'] . '0a30e26e83b2ac5b9e29e1b161e5c1fa742');		
		$new_values[$row['id']] = $new_password;
	}
} 
$return = array();
foreach($new_values as $key=>$pass){	
	getres("UPDATE user SET  password='$pass' WHERE id='$key'");
}
die('123654');*/
