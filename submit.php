...
if($_POST['newPasswordB'] != ""){
		
	$token = $_POST['token'];	
	$uid = getraw('user','id',"tokenPass='".$token."'");
	$uname = getraw('user','uname',"id='".$uid."'");
	
	if(strlen($_POST['newPasswordB'])<6){
        $formerrors.='The password must be at least 6 characters long. <br />';
	}
    if($_POST['newPasswordB'] != $_POST['newPasswordB']){
        $formerrors.='The password and retyped password do not match. <br />';
	}
	/*create new password*/
	$password = sha1($_POST['newPasswordB'] . '0a30e26e83b2ac5b9e29e1b161e5c1fa742');
    
	if($formerrors=='')
    {
        setraw('user','password',$uid,$password,$null);
        $formerrors.="OK - Password changed!<br>";		
...
if($_POST['newuname']!="") {
    ...
    if ($resp->is_valid)
    {
        ...
        if(getraw('user','email',"uname='".mysql_real_escape_string  ($_POST['email'])."'",$null)!=''){
    ...
    if($formerrors=='')
    {
		...
		$new_changed_password = sha1($password . '0a30e26e83b2ac5b9e29e1b161e5c1fa742');
		
        getres("INSERT INTO toactivate (password,token,uid) VALUES ('".$new_changed_password."','$token',$nuid)");
		
        $subject="Activate account at ".$settings['sitename'];		
        /*$activurl=$settings['siteURL']."activate.php?token=".$token;*/
        $activurl="http://visionsforchangenorwich.org.uk/activate.php?token=".$token;
...
if($_POST['emailofforgotten']) {
    ...
    if ($resp->is_valid)
    {
        ...
        {	
			...
			/*update key in user field*/
			setraw('user','tokenPass',$fuserid,$token,$null);
			
			/*$link = $settings['siteURL']."chgpassword.php?token=".$token;*/
			$link = "http://visionsforchangenorwich.org.uk/chgpassword.php?token=".$token;
			
            $plainmessage='Please click here: '.$link.' to change your password.'; 
...
/*change the password from the user pages*/
if($_POST['replacementpassword']) {
    getraw('user','uname',"id='".$uid."' And password='".mysql_real_escape_string(stripslashes(sha1($_POST['oldpassword'] . '0a30e26e83b2ac5b9e29e1b161e5c1fa742')))."'",$fail);
    if($fail){
	...
	/*create new password*/
    $password = mysql_real_escape_string(stripslashes(sha1($_POST['replacementpassword'] . '0a30e26e83b2ac5b9e29e1b161e5c1fa742')));
...
//NEW HANDLERS SHOULD GO ABOVE THIS SECTION - NOT HERE
?>